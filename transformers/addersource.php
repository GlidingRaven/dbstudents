<?php
	...
	$ex = mysql_query("UPDATE `campuses` SET count_sources = count_sources + 1 WHERE code_UZ = $code_uz");//Обновляем счётчик сорсов
	if ($ex==false) {echo "1st eror";exit();}

	$ex = mysql_query("INSERT INTO `sources` (`date`, `code_source`, `code_UZ`, `abb_name_UZ`, `link`, `count_students`) VALUES ('$date_source', '$code_source', '$code_uz', '$name_uz', '$url_source', '$count_students')");//Добавляем сорс
